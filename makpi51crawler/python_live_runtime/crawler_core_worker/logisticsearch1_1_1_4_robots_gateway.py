    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            FROM http_fetch.upsert_robots_txt_cache(
                p_host_id => %(host_id)s::bigint,
                p_robots_url => %(robots_url)s::text,
                p_cache_state => %(cache_state)s::http_fetch.robots_cache_state_enum,
                p_http_status => %(http_status)s::integer,
                p_fetched_at => %(fetched_at)s::timestamp with time zone,
                p_expires_at => %(expires_at)s::timestamp with time zone,
                p_etag => %(etag)s::text,
                p_last_modified => %(last_modified)s::text,
                p_raw_storage_path => %(raw_storage_path)s::text,
                p_raw_sha256 => %(raw_sha256)s::text,
                p_raw_bytes => %(raw_bytes)s::bigint,
                p_parsed_rules => %(parsed_rules)s::jsonb,
                p_sitemap_urls => %(sitemap_urls)s::jsonb,
                p_crawl_delay_seconds => %(crawl_delay_seconds)s::numeric,
                p_error_class => %(error_class)s::text,
                p_error_message => %(error_message)s::text,
