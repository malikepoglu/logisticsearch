# EN: This module is the interpreted "poweroff preparation" control surface.
# EN: It intentionally performs only the safe internal durable stop phase.
# TR: Bu modül yorumlanan "poweroff hazırlık" kontrol yüzeyidir.
...
# EN: matching the previous shell behavior.
# TR: Bilgilendirici takip satırları stderr'e gitmelidir ve bu önceki shell
# TR: davranışıyla uyumlu olmalıdır; bu yüzden sys içe aktarıyoruz.
import sys
...
# EN: This callable requests durable stop preparation for the later poweroff phase.
# TR: Bu çağrılabilir sonraki poweroff aşaması için kalıcı stop hazırlığını ister.
def main() -> int:
    ...
    # EN: Real poweroff is intentionally not implemented here yet.
    # TR: Gerçek poweroff burada henüz bilinçli olarak uygulanmamıştır.
    print(
        "INFO: poweroffwc currently performs only the safe internal stop-preparation phase.",
        file=sys.stderr,
    )
    print(
        "INFO: real poweroff phase is intentionally not implemented in this step.",
        file=sys.stderr,
    )
    return 0
